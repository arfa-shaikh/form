...
class Register:
    ...
    def __init__(self, root):
        ...
        self.name = Entry(root, textvariable=self.nameValue,
                          width=30, bd=2, font=20)
        self.name.place(x=300, y=400)

        Label(text="Phone no:", font=23).place(x=200, y=450)
        self.phoneValue = StringVar()
        self.phone = Entry(root, textvariable=self.phoneValue,
                           width=30, bd=2, font=20)
        ...
        self.gender = ttk.Combobox(
            root, textvariable=self.genderValue, width=25, font=20, state="readonly")
        ...
        self.email = Entry(root, textvariable=self.emailValue,
                           width=30, bd=2, font=20)
        self.email.place(x=300, y=550)

        checkValue = IntVar
        checkbtn = Checkbutton(
            text="DO YOU AGREE THAT THE ENTERED INFORMATION IS CORRECT?", variable=checkValue)
        checkbtn.place(x=200, y=600)

        Button(root, text="Submit", font=20, width=11, height=2, cursor="hand1", command=lambda: [
               self.register_data(), self.view_data(), self.delete_entry()]).place(x=350, y=670)
    ...
    def register_data(self):
        ...
        if self.name.get() == "" or self.phone.get() == "" or self.email.get() == "" or self.gender.get() == "":
            ...
            mydb = mysql.connector.connect(
                host="127.0.0.1", user="root", passwd="12345", database="gui")
            cur = mydb.cursor()
...
#
    ...
    def view_data(self):
        ...
        # place(x=650, y=120, width=470, height=300)
        tree_view = ttk.Treeview(self.root, columns=(
            1, 2, 3, 4, 5), show="headings", height="10")
        ybar = tkinter.Scrollbar(
            self.root, orient=tkinter.VERTICAL, command=tree_view.yview)
        tree_view.configure(yscroll=ybar.set)

        tree_view.column(1, stretch=NO, width=40)
        tree_view.heading(1, text="Customer ID")
        ...
        tree_view.pack(side=TOP, fill=BOTH)
